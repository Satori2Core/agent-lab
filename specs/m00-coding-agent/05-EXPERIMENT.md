...
Module 3（Tool 系统）和 Module 5（Agent Loop）的关联远比我们想的更紧密：
- Agent 的循环逻辑（Module 5）依赖于工具（Module 3）的可靠性
- 如果 Tool 不可靠，Agent Loop 再精致也没用
- **一个好的 Tool 必须：** ① 输入输出明确 ② 原子操作 ③ 不依赖上下文猜测

```go
// 这个认知会直接影响 Module 3 的 Tool 接口设计：
...
# git checkout -- specs/m00-coding-agent/experiment/feedback_loop/bad.go
```
